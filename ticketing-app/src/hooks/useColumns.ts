...
export function useColumns() {
  ...
  const handleColumnDoubleClick = (tabId: string, columnId: string, tables: Record<string, any>) => {
  ...
  const saveColumnName = (tables: Record<string, any>, setTables: (tables: Record<string, any>) => void) => {
  ...
  const handleColumnDrop = (
    ...
    const draggedColumnIndex = table.columns.findIndex((col: any) => col.id === draggedColumn)
    const targetColumnIndex = table.columns.findIndex((col: any) => col.id === targetColumnId)
